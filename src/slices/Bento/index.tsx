...
const Bento: FC<BentoProps> = ({ slice }) => {
  return (
    ...
    components={{
      ...
      em:({children}) => (
        <em className="bg-linear-to-b from-yellow-100 to-yellow-500 bg-clip-text not-italic text-transparent">
          {children}
        </em>
      ),
    }}
    ...
    {slice.primary.title.map((item) => (
  <div className={clsx("glass-container row-span-3 grid grid-rows-subgrid gap-4 rounded-lg bg-linear-to-b from-gray-900 to-gray-950 p-4", item.wide? "md:col-span-2" : "md:col-span-1")} key={asText(item.title)}>
    ...
    <PrismicNextImage 
    field={item.image}
    alt={(item.image?.alt ? item.image.alt : "") as ""}
