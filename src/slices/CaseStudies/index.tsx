 ...
 */
const CaseStudies: FC<CaseStudiesProps> = async ({ slice }): Promise<JSX.Element> => {

  const client = createClient();

  const caseStudies = await Promise.all(
    slice.primary.item.map(async (item)=>{
        ...
        {caseStudies.map((caseStudy, index) =>(
          caseStudy && (
            <div key={caseStudy.id} className="relative grid gap-4 opacity-85 transition-opacity duration-300 hover:cursor-pointer hover:opacity-100 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
              ...
              className="after:absolute after:inset-0 hover:underline text-center md:text-left" > 
